export default class FormValidator{
    ...
    _setEventListeners(this._settings,this._iterarFormElement){
            this._inputs= Array.from (iterarFormElement.querySelectorAll(settings.inputSelector))
            this._buttonElement= this._iterarFormElement.querySelector(settings.submitButtonSelector)
    ...
    enableValidation(this._settings){
        ...
        this._forms.forEach(function(this._iterarFormElement){ //aqui empezamos a iterar en los elementos con for each
        this._iterarFormElement.addEventListener("submit",function(evt){     //aqui solo se esta dando submit a 1 solo formulario
