...
if(hasInvalidInput(inputs)){   //esta condicional es para validar mas organizado si el input es invalido
...
function hideInputError(settings,iterarFormElement,iterarInput){
    const errorElement=iterarFormElement.querySelector(`#${iterarInput.id}-error`)
...
function showInputError(settings,iterarFormElement,iterarInput){
  ...
  const errorElement=iterarFormElement.querySelector(`#${iterarInput.id}-error`)//los back sticks son una comilla especial para poder concatenar una expresion de js con texto
...
function setEventListeners(settings,iterarFormElement){
    const inputs=Array.from (iterarFormElement.querySelectorAll(settings.inputSelector))
    const buttonElement=iterarFormElement.querySelector(settings.submitButtonSelector)
   toggleButtonState(settings,inputs,buttonElement)  //el toggle significa que cambia de prendido a apagado
    inputs.forEach(function(iterarInput){  //aqui estamos iterando en 1 input solamente
...
function enableValidation(settings){
    ...
    forms.forEach(function(iterarFormElement){ //aqui empezamos a iterar en los elementos con for each
